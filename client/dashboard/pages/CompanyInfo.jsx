...
const CompanyInfoDashboard = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get(
        "http://localhost:8090/api/v1/companyinfo/history"
      );
      setHistory(res.data.data);
    } catch (err) {
      console.error("Error fetching history:", err);
  ...
  const deleteHistoryItem = async (id) => {
    if (window.confirm("Are you sure you want to delete this history entry?")) {
      try {
        await axios.delete(
          `http://localhost:8090/api/v1/companyinfo/history/${id}`
        );
        fetchHistory();
      } catch (err) {
        setError(err.message);
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-4">
                {history.map((item) => (
                  <div
                    key={item._id}
                    className="border border-stone-200 p-4 rounded-lg bg-white"
                  >
                    <div className="flex justify-between items-start mb-2">
          ...
          <>
            ...
            {editMode ? (
              <form
                ...
                className="bg-white p-6 rounded-lg shadow-md border border-stone-200"
              >
                {activeTab === "about" ? (
                  <>
                    <h2 className="text-xl font-semibold mb-4 text-stone-900">
